...
export default function ProjectFormStepLine() {
  ...
  return (
    <div className="bg-secondary h-full p-6  w-[30%] ">
      ...
      <ol className="relative space-y-6 my-6">
        {steps_info.map(({ label, description, completed }, index) => (
          <li key={index + label} className="flex items-start">
            {completed ? (
              <button onClick={()=>JumpFormStep(index)} className={clsx("flex-shrink-0 w-8 h-8 bg-primary rounded-full flex items-center justify-center relative",active_count==index&&"!bg-gray-600")}>
                <CheckIcon className="w-5 h-5 text-white" />
              </button>
            ) : (
              <div
                className={clsx(
                  "flex-shrink-0 w-8 h-8 bg-gray-300 rounded-full flex items-center justify-center text-gray-500 font-medium relative before:content-[''] before:absolute before:top-[-24px] before:left-1/2 before:transform before:-translate-x-1/2 before:w-px  before:bg-gray-500",
                  active_count == index &&
