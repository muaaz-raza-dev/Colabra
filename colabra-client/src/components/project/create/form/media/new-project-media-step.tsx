...
import NewProjectMediaUpload from './new-project-media-upload'

export default function NewProjectMediaStep({label}:{label:string}) {
  const {setValue,getValues,watch} = useFormContext<InewProjectForm>()
  const uploaded_images = watch("payload.images.urls")
  const [samples,setSamples] =useState<string[]>(uploaded_images)
  useEffect(() => {
    setSamples(uploaded_images)
  }, [uploaded_images])
  ...
  <div className="flex flex-wrap gap-3">
        ...
        <NewProjectMediaUpload label={label}/>
</section>
  )
}
